<template>
    <section class="mt-[10px] 2xs:px-[24px] pt-[24px] bg-[#D7E6FE] rounded-[20px]">
        <div class="flex flex-wrap">
            <div class="lg:flex-33.33% lg:pr-[16px]">
                <div
                    class="pt-[12px] px-[16px] pb-[16px] border-2 border-white border-dashed rounded-t-[14px] rounded-br-[14px]">
                    <p class="text-[16px] leading-[27.5px] tracking-[0.16px]">
                        Oyafly has completely transformed the way I travel within Nigeria. Their seamless online booking
                        platform and excellent customer service have made booking flights a breeze.
                    </p>
                </div>
                <div class="mt-2 inline-block">
                    <div class="py-2 pl-[16px] pr-2 border-2 border-white border-dashed rounded-[14px] flex items-center">
                        <span class="text-[14px] tracking-[0.28px] font-medium text-of-dark/60">Oluwafemi Daniels</span>
                        <img src="@/assets/images/about/feedback/1.png" alt="" class="ml-[24px]">
                    </div>
                </div>
            </div>
            <div class="lg:flex-33.33% lg:pr-[16px] mt-8 lg:mt-0">
                <div
                    class="lg:mt-[160px] pt-[12px] px-[16px] pb-[16px] border-2 border-white border-dashed rounded-t-[14px] rounded-br-[14px]">
                    <p class="text-[16px] leading-[27.5px] tracking-[0.16px]">
                        I love the convenience and reliability they offer, and their commitment to making travel accessible
                        to all Nigerians is truly commendable.
                    </p>
                </div>
                <div class="mt-2 inline-block">
                    <div class="py-2 pl-[16px] pr-2 border-2 border-white border-dashed rounded-[14px] flex items-center">
                        <span class="text-[14px] tracking-[0.28px] font-medium text-of-dark/60">Mirabel Effiong</span>
                        <img src="@/assets/images/about/feedback/2.png" alt="" class="ml-[24px]">
                    </div>
                </div>
            </div>
            <div class="lg:flex-33.33% mt-8 lg:mt-0">
                <div
                    class="lg:mt-[320px] pt-[12px] px-[16px] pb-[16px] border-2 border-white border-dashed rounded-t-[14px] rounded-br-[14px]">
                    <p class="text-[16px] leading-[27.5px] tracking-[0.16px]">
                        Oyafly has become my go-to platform for booking flights, and I highly recommend it to anyone looking
                        for a hassle-free travel experience.
                    </p>
                </div>
                <div class="mt-2 inline-block">
                    <div class="py-2 pl-[16px] pr-2 border-2 border-white border-dashed rounded-[14px] flex items-center">
                        <span class="text-[14px] tracking-[0.28px] font-medium text-of-dark/60">Yetunde Nwosu</span>
                        <img src="@/assets/images/about/feedback/3.png" alt="" class="ml-[24px]">
                    </div>
                </div>
            </div>
        </div>
        <div class="mt-[40px] mb-[80px]">
            <h1 class="text-[34px] 2xs:text-[40px] xs:text-[52px] sm:text-[48px] leading-[46px] font-semibold font-pp max-w-[485px] tracking-[-0.96px]">
                Cheers from our users
            </h1>
            <p
                class="mt-[12px] text-[18px] sm:text-[24px] leading-[22px] sm:leading-[36px] max-w-[460px] tracking-[0.24px] text-of-dark/60">
                Discover the stories that make us smile
                – hear it straight from our happy users.
            </p>

        </div>
        <img src="@/assets/images/about/feedback/pattern.svg" alt="" class="">
    </section>
</template>